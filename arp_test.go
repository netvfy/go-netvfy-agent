...
func TestArpTable(t *testing.T) {
	...
	err = arpTable.Update(testIP, testMACHW, time.Now())
	if err != nil {
	...
	err = arpTable.Update(testIP2, testMACHW2, time.Now())
	if err != nil {
...
func TestArpTablePurge(t *testing.T) {
	...
	// Update ArpEntry 1 and and check it exists
	err = arpTable.Update(testIP, testMACHW, arpTime)
	if err != nil {
		t.Fatalf("unable to add testIP2: %v", err)
	}
	...
	err = arpTable.Update(testIP2, testMACHW2, arpTime)
	if err != nil {
		t.Fatalf("unable to update arp entry for testIP2: %v", err)
	}
